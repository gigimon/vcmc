...
[] Добавить верхнюю строку меню с группами действий (`Left`, `Files`, `Command`, `Options`, `Right`).
[] Вынести подключение SFTP в пункт меню (`Command -> Connect SFTP`) с сохранением существующего `F9`.
[] Добавить в меню входные точки для новых функций (Find, Archive VFS, Viewer modes, Editor settings).
[] Реализовать клавиатурную навигацию меню (`F9/Alt+буква`, стрелки, Enter, Esc) без регрессий в основном режиме.
...
[] Реализовать асинхронный запуск `fd` через job/workers с live-progress в status/footer.
[] Показать результаты в виртуальной панели (panelized view) с переходом к исходному пути.
[] Сделать fallback: если `fd` не найден, показать понятный диалог с подсказкой по установке.
