...
# Description (итерация 6)
Цель: добавить функции ежедневной практической работы с файлами и серверами: поиск по содержимому, быстрые подключения к сохраненным SFTP-хостам и удобное визуальное управление правами/атрибутами.

Принятые решения:
- Поиск по содержимому строится на внешней утилите `rg` (`ripgrep`) с panelized results и переходом к файлу/позиции.
- SFTP bookmarks хранятся в пользовательском конфиге (`XDG config`/`~/.config/vcmc`) и доступны из верхнего меню.
- Редактор прав и атрибутов делается как модальный визуальный диалог с preview изменений перед применением.

Нефункциональные требования:
- Любой долгий поиск выполняется асинхронно без блокировки UI.
- Данные bookmarks и сохраненные настройки должны быть обратимо редактируемы и устойчивы к битым записям (fallback + понятная ошибка).
- Изменение прав/атрибутов должно иметь безопасные подтверждения и прозрачные сообщения об ошибках.
...
[] Добавить хранение bookmarks в конфиге (`XDG config`/`~/.config/vcmc`) с загрузкой при старте.
[] Реализовать меню bookmarks: быстрый connect, добавить текущий, переименовать, удалить.
[] Поддержать авто-подстановку bookmark в flow подключения (`Left/Right -> Connect SFTP`).

## Step 37: Визуальный редактор прав и атрибутов
[] Добавить диалог свойств выбранного файла/директории (owner/group/mode/mtime где доступно).
[] Реализовать визуальный chmod-редактор (`rwx` для `u/g/o`, числовой preview `0755`).
[] Добавить операции изменения атрибутов (`chmod`, `touch`/mtime; owner/group где разрешено) с confirm.
